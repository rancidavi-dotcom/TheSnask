...
pub fn get_packages_dir() -> PathBuf {
    ...
    if !dir.exists() {
        fs::create_dir_all(&dir).expect("Failed to create packages directory.");
    }
    dir
}
...
pub fn get_frameworks_dir() -> PathBuf {
    let home = dirs::home_dir().expect("Could not find the home directory.");
    let dir = home.join(".snask").join("frameworks");
    if !dir.exists() {
        fs::create_dir_all(&dir).expect("Failed to create frameworks directory.");
    }
    dir
}

pub fn install_framework(name: &str) -> Result<(), String> {
    let frameworks_dir = get_frameworks_dir();
    let dest_dir = frameworks_dir.join(name);

    if dest_dir.exists() {
        println!("🔄 Framework '{}' já está instalado. Atualizando...", name);
        let status = Command::new("git")
            .arg("pull")
            .current_dir(&dest_dir)
            .status()
            .map_err(|e| format!("Falha ao atualizar framework via git pull: {}", e))?;
        if !status.success() {
            return Err(format!("git pull falhou para o framework '{}'.", name));
        }
    } else {
        println!("🔍 Instalando framework '{}'...", name);
        let url = match name {
            "zenith" => "https://github.com/rancidavi-dotcom/Zenith-Framework.git",
            _ => return Err(format!("Framework '{}' desconhecido.", name)),
        };

        let status = Command::new("git")
            .args(["clone", "--depth", "1", url, dest_dir.to_str().unwrap()])
            .status()
            .map_err(|e| format!("Falha ao clonar framework via git: {}", e))?;
        if !status.success() {
            return Err(format!("git clone falhou para o framework '{}'.", name));
        }
    }
...
pub fn fetch_registry() -> Result<Registry, String> {
    ...
    match ensure_registry_repo().and_then(|repo| read_registry_from_repo(&repo)) {
        ...
        Err(git_err) => {
            ...
            response
                .json()
                .map_err(|e| format!("Failed to parse registry.json (HTTP): {}", e))
        }
    }
}
...
pub fn install_package_with_registry(name: &str, registry: &Registry) -> Result<(String, String, PathBuf), String> {
    ...
    fs::write(&dest_path, &content)
        .map_err(|e| format!("Failed to save package locally: {}", e))?;
...
pub fn uninstall_package(name: &str) -> Result<(), String> {
    ...
    if path.exists() {
        fs::remove_file(&path).map_err(|e| format!("Failed to remove package: {}", e))?;
        println!("🗑️  Package '{}' uninstalled successfully.", name);
        Ok(())
    } else {
