...
fn main() {
    ...
    let result = match &cli.command {
        ...
        Commands::Build { file, output, target, lto, release_size, min_runtime, tiny, extreme } => {
            run_build(file, output, target, *lto, *release_size, *min_runtime, *tiny, *extreme)
        }
        Commands::Run { file } => {
            run_program(file)
        }
        Commands::Add { name, version } => {
            sps::add_dependency(name, version.clone())
        }
        Commands::Remove { name } => {
            sps::remove_dependency(name)
        }
        Commands::Setup { target } => {
            tools::run_setup(target.clone())
        }
        ...
        Commands::Dist { file, targets, all, deb, appimage, name, linux_user, out_dir } => {
            dist::run_dist(file.clone(), targets.clone(), *all, *deb, *appimage, name.clone(), *linux_user, out_dir.clone())
        }
        Commands::Doctor => {
            tools::doctor()
        }
...
fn run_program(file: &Option<String>) -> Result<(), String> {
        ...
        if !arg.ends_with(".snask") {
            if let Ok((m, _)) = sps::load_manifest_from(&cwd) {
                if let Some(cmdline) = m.scripts.get(arg) {
                    let status = Command::new("sh").arg("-lc").arg(cmdline).status().map_err(|e| e.to_string())?;
                    if !status.success() {
    ...
    let status = Command::new(&binary_path).status().map_err(|e| format!("Failed to run binary: {}", e))?;
    if !status.success() {
