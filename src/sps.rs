...
pub struct BuildSection {
    ...
    /// Optional LTO mode: off|thin
    #[serde(default)]
    pub lto: Option<String>,
    /// Optional size-oriented opt flag: O2|O3|Os|Oz
    #[serde(default)]
...
impl SpsManifest {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if let Some(lto) = &self.build.lto {
            if lto != "off" && lto != "thin" {
                return Err("SPS: build.lto must be 'off' or 'thin'".to_string());
            }
        }
        if let Some(opt) = &self.build.opt {
            let ok = opt == "O2" || opt == "O3" || opt == "Os" || opt == "Oz";
            if !ok {
                return Err("SPS: build.opt must be one of: O2, O3, Os, Oz".to_string());
            }
        }
        Ok(())
    }
...
fn manifest_from_snif(src: &str) -> Result<SpsManifest, String> {
    ...
    let features = match build_obj.get("features") {
        None => BTreeMap::new(),
        Some(f) => {
            let f_obj = snif_get_obj(f, "build.features")?;
            let mut out = BTreeMap::new();
            for (k, v) in f_obj {
                let feature_val = match v {
                    SnifValue::Bool(b) => SnifFeatureValue::Bool(*b),
                    SnifValue::String(s) => SnifFeatureValue::String(s.clone()),
                    SnifValue::Number(n) => SnifFeatureValue::Number(*n),
                    _ => return Err(format!("Invalid value type in build.features.{}. Must be a string, number, or boolean.", k)),
                };
                out.insert(k.clone(), feature_val);
            }
            out
        }
    };
    let build = BuildSection {
        opt_level: snif_get_u8(build_obj, "opt_level", default_opt_level())?,
        features,
        profile: match build_obj.get("profile") {
            Some(SnifValue::String(s)) => Some(s.clone()),
            Some(_) => return Err("Expected string in build.profile".to_string()),
            None => None,
        },
        strip: match build_obj.get("strip") {
            Some(SnifValue::Bool(b)) => Some(*b),
            Some(_) => return Err("Expected bool in build.strip".to_string()),
            None => None,
        },
        lto: match build_obj.get("lto") {
            Some(SnifValue::String(s)) => Some(s.clone()),
            Some(_) => return Err("Expected string in build.lto".to_string()),
            None => None,
        },
        opt: match build_obj.get("opt") {
            Some(SnifValue::String(s)) => Some(s.clone()),
            Some(_) => return Err("Expected string in build.opt".to_string()),
            None => None,
        },
    };
...
pub fn init_project(name: Option<String>) -> Result<(), String> {
    ...
    let manifest = format!(
        "{{\n  package: {{ name: \"{}\", version: \"0.1.0\", entry: \"{}\", }},\n  dependencies: {{}},\n  build: {{ opt_level: 2, profile: \"release-size\", strip: true, lto: \"thin\", opt: \"Os\", }},\n}}\n",
...
pub fn resolve_deps_and_lock(dir: &std::path::Path, manifest: &SpsManifest) -> Result<(), String> {
    let registry = crate::packages::fetch_registry()?;
...
pub fn pin_from_lock(dir: &std::path::Path, manifest: &SpsManifest) -> Result<(), String> {
    ...
    let lock_path = lockfile_path(dir);
    if !lock_path.exists() {
        return Ok(());
    }
    let lock = read_lockfile(dir)?;
    let registry = crate::packages::fetch_registry()?;
