...
// ------------------------------------------------------------
// Sjson text format (SJSON)
// Goal: easier-than-JSON syntax + very fast parsing.
//
// Features:
// - Unquoted keys: {a:1, user_name:"x"}
// - Single or double quoted strings: 'hi' or "hi"
// - Trailing commas in objects/arrays
// - Line comments: // ... and # ...
// - nil/null, true/false
//
// Security:
// - Depth limit + max input length
// - Duplicate keys are rejected (prevents shadowing attacks)
// ------------------------------------------------------------

// Forward declarations from the runtime (defined later in this file).
static SnaskValue make_num(double n);
static SnaskValue make_bool(bool b);
static SnaskValue make_str(char* s);
static SnaskValue make_obj(SnaskObject* o);
...
typedef struct {
    ...
    size_t max_len;
    const char* err;
    // Reference table: &name <value> to define, *name to use.
    // Values are returned by reference (same underlying pointer).
    char** ref_names;
...
static SnaskValue sjson_make_tagged(const char* key, SnaskValue val) {
    SnaskObject* o = obj_new(1);
    o->names[0] = snask_gc_strdup((char*)key);
    o->values[0] = val;
    return make_obj(o);
}
...
static void sjson_ref_set(SjsonP* p, char* name, SnaskValue v) {
    ...
    if (p->ref_count + 1 > p->ref_cap) {
        ...
        if (!nn || !nv) { p->err = "Out of memory while storing SJSON references."; return; }
        p->ref_names = nn;
        p->ref_values = nv;
        p->ref_cap = nc;
    }
...
static SnaskValue sjson_ref_get(SjsonP* p, const char* name) {
    ...
    for (int i = 0; i < p->ref_count; i++) {
        if (p->ref_names[i] && strcmp(p->ref_names[i], name) == 0) {
            return p->ref_values[i];
        }
    }
    p->err = "Unknown SJSON reference (use &name before *name).";
    return make_nil();
}
...
typedef struct { char* data; size_t len; size_t cap; } SjsonBuf;
static void sjsonb_init(SjsonBuf* b) {
    b->cap = 256; b->len = 0; b->data = (char*)malloc(b->cap);
    if (b->data) b->data[0] = 0;
}
static void sjsonb_push_char(SjsonBuf* b, char c) {
    if (!b->data) return;
    if (b->len + 2 >= b->cap) {
        size_t nc = b->cap * 2;
        char* np = (char*)realloc(b->data, nc);
        if (!np) return;
        b->data = np; b->cap = nc;
    }
    b->data[b->len++] = c;
...
static SnaskValue sjson_parse_string(SjsonP* p) {
    sjson_skip_ws_and_comments(p);
    if (p->i >= p->len) { p->err = "Unexpected end of input while parsing string."; return make_nil(); }
    char q = p->s[p->i];
    if (q != '"' && q != '\'') { p->err = "Expected string literal."; return make_nil(); }
    p->i++;
    size_t start = p->i;
    ...
    if (p->i < p->len && p->s[p->i] == q) {
        size_t n = p->i - start;
        p->i++;
        return make_str(snask_gc_strndup(p->s + start, n));
    }

    // Slow path with escapes
    SjsonBuf sb; sjsonb_init(&sb);
    while (p->i < p->len) {
        char c = p->s[p->i++];
        if (c == q) {
            return make_str(sb.data);
        }
        if (c == '\\') {
            if (p->i >= p->len) { p->err = "Unterminated escape sequence in string."; return make_nil(); }
            char e = p->s[p->i++];
            switch (e) {
                case 'n': sjsonb_push_char(&sb, '\n'); break;
                case 'r': sjsonb_push_char(&sb, '\r'); break;
                case 't': sjsonb_push_char(&sb, '\t'); break;
                case '\\': sjsonb_push_char(&sb, '\\'); break;
                case '"': sjsonb_push_char(&sb, '"'); break;
                case '\'': sjsonb_push_char(&sb, '\''); break;
                default: {
                    sjsonb_push_char(&sb, e);
                } break;
            }
            continue;
        }
        if (c == '\n' || c == '\r') { p->err = "Unterminated string literal (newline)."; return make_nil(); }
        sjsonb_push_char(&sb, c);
    }
    p->err = "Unterminated string literal (end of file).";
    return make_nil();
}

static char* sjson_parse_key(SjsonP* p) {
    sjson_skip_ws_and_comments(p);
    if (p->i >= p->len) { p->err = "Unexpected end of input while parsing key."; return NULL; }
    char c = p->s[p->i];
    if (c == '"' || c == '\'') {
        SnaskValue sv = sjson_parse_string(p);
        if ((int)sv.tag != SNASK_STR || !sv.ptr) return NULL;
        return (char*)sv.ptr;
    }
    if (!sjson_is_key_start(c)) {
        p->err = "Expected object key (identifier or quoted string).";
        return NULL;
    }
    size_t start = p->i;
    p->i++;
...
static bool sjson_obj_has_key(SnaskObject* o, const char* k) {
    if (!o || !k) return false;
    for (int i = 0; i < o->count; i++) {
        if (o->names[i] && strcmp(o->names[i], k) == 0) return true;
    }
    return false;
}
...
static SnaskValue sjson_parse_array(SjsonP* p) {
    ...
    for (;;) {
        ...
        if (p->err) { free(arr); return make_nil(); }
        char idx_name[32];
        snprintf(idx_name, sizeof(idx_name), "%d", len);
        obj_push(&arr, &cap, &len, snask_gc_strdup(idx_name), v);
        sjson_skip_ws_and_comments(p);
        if (sjson_consume(p, ']')) { p->depth--; return make_obj(arr); }
        // allow trailing comma
        if (sjson_consume(p, ',')) { sjson_skip_ws_and_comments(p); if (sjson_consume(p, ']')) { p->depth--; return make_obj(arr); } continue; }
...
static SnaskValue sjson_parse_object(SjsonP* p) {
    ...
    if (!sjson_consume(p, '{')) { p->err = "Expected '{'."; return make_nil(); }
    SnaskObject* obj = (SnaskObject*)malloc(sizeof(SnaskObject));
    obj->count = 0; obj->names = NULL; obj->values = NULL;
    int cap = 0, len = 0;
    sjson_skip_ws_and_comments(p);
    if (sjson_consume(p, '}')) { p->depth--; return make_obj(obj); }
    for (;;) {
        char* key = sjson_parse_key(p);
        if (p->err) { free(obj); return make_nil(); }
        if (sjson_obj_has_key(obj, key)) { p->err = "Duplicate key in object."; free(obj); return make_nil(); }
        sjson_skip_ws_and_comments(p);
        if (!sjson_consume(p, ':')) { p->err = "Expected ':' after key."; free(obj); return make_nil(); }
        SnaskValue v = sjson_parse_value(p);
        if (p->err) { free(obj); return make_nil(); }
        obj_push(&obj, &cap, &len, key, v);
        sjson_skip_ws_and_comments(p);
        if (sjson_consume(p, '}')) { p->depth--; return make_obj(obj); }
        if (sjson_consume(p, ',')) { sjson_skip_ws_and_comments(p); if (sjson_consume(p, '}')) { p->depth--; return make_obj(obj); } continue; }
...
static SnaskValue sjson_parse_number(SjsonP* p) {
    sjson_skip_ws_and_comments(p);
    size_t start = p->i;
    ...
    while (p->i < p->len) {
        char c = p->s[p->i];
        if (c >= '0' && c <= '9') { any = true; p->i++; continue; }
        break;
    }
    ...
    if (!any) { p->err = "Invalid number literal."; return make_nil(); }
    // exponent
    if (p->i < p->len && (p->s[p->i] == 'e' || p->s[p->i] == 'E')) {
        ...
        while (p->i < p->len) {
            char c = p->s[p->i];
            if (c >= '0' && c <= '9') { exp_any = true; p->i++; continue; }
            break;
        }
        if (!exp_any) { p->i = epos; } // rollback exponent
    }
    char* tmp = snask_gc_strndup(p->s + start, p->i - start);
    if (is_float) {
        return make_num(strtod(tmp, NULL));
    }

    // Integer path: avoid JSON "number ambiguity" and precision loss beyond 2^53-1.
    // - If within safe IEEE-754 integer range, return SNASK_NUM.
    // - Otherwise, return tagged object: { "$i64": "..." } (string) for exactness.
    errno = 0;
    char* endp = NULL;
    long long v = strtoll(tmp, &endp, 10);
    if (errno == 0 && endp && *endp == 0) {
        const long long SAFE_MAX = 9007199254740991LL; // 2^53-1
        if (v <= SAFE_MAX && v >= -SAFE_MAX) {
            return make_num((double)v);
        }
        return sjson_make_tagged("$i64", make_str(tmp));
    }
    // fallback
    return make_num(strtod(tmp, NULL));
}
...
static SnaskValue sjson_parse_value(SjsonP* p) {
    sjson_skip_ws_and_comments(p);
    if (p->i >= p->len) { p->err = "Unexpected end of input."; return make_nil(); }
    char c = p->s[p->i];
    if (c == '{') return sjson_parse_object(p);
    if (c == '[') return sjson_parse_array(p);
    if (c == '"' || c == '\'') return sjson_parse_string(p);
    // References:
    // - &name <value> defines a reference (name -> value)
    // - *name returns an existing reference (same underlying pointer)
    if (c == '&') {
        p->i++;
        sjson_skip_ws_and_comments(p);
        if (p->i >= p->len || !sjson_is_key_start(p->s[p->i])) { p->err = "Expected reference name after '&'."; return make_nil(); }
        size_t st = p->i;
        p->i++;
        while (p->i < p->len && sjson_is_key_char(p->s[p->i])) p->i++;
        char* name = snask_gc_strndup(p->s + st, p->i - st);
        SnaskValue v = sjson_parse_value(p);
        if (p->err) return make_nil();
        sjson_ref_set(p, name, v);
        return v;
    }
    if (c == '*') {
        p->i++;
    ...
    // Typed values (fixes JSON's weak type system):
    // - @date"2026-01-01T12:00:00Z"  -> { "$date": "..." }
    // - @dec"12.34"                  -> { "$dec": "..." } (exact decimal)
    // - @bin"BASE64..."              -> { "$bin": "..." }
    // - @enum"STATUS_OK"             -> { "$enum": "..." }
    if (c == '@') {
        p->i++;
        sjson_skip_ws_and_comments(p);
        if (p->i >= p->len || !sjson_is_key_start(p->s[p->i])) { p->err = "Expected type name after '@'."; return make_nil(); }
        size_t st = p->i;
        p->i++;
        while (p->i < p->len && sjson_is_key_char(p->s[p->i])) p->i++;
        size_t n = p->i - st;
        const char* tn = p->s + st;
        SnaskValue sv = sjson_parse_string(p);
        if (p->err) return make_nil();
        if ((int)sv.tag != SNASK_STR || !sv.ptr) { p->err = "Expected string literal after '@type'."; return make_nil(); }
        if (n == 4 && strncmp(tn, "date", 4) == 0) return sjson_make_tagged("$date", sv);
        if (n == 3 && strncmp(tn, "dec", 3) == 0) return sjson_make_tagged("$dec", sv);
        if (n == 3 && strncmp(tn, "bin", 3) == 0) return sjson_make_tagged("$bin", sv);
        if (n == 4 && strncmp(tn, "enum", 4) == 0) return sjson_make_tagged("$enum", sv);
        // Unknown type: keep as tagged { "$type": "...", "value": "..." }
        SnaskObject* o = obj_new(2);
        o->names[0] = snask_gc_strdup("$type"); o->values[0] = make_str(snask_gc_strndup(tn, n));
        o->names[1] = snask_gc_strdup("value"); o->values[1] = sv;
        return make_obj(o);
    }
    if ((c >= '0' && c <= '9') || c == '-' || c == '+') return sjson_parse_number(p);
    ...
    if (sjson_is_key_start(c)) {
        ...
        p->err = "Barewords are not allowed. Use a quoted string (\"...\") or a typed literal like @enum\"NAME\".";
        return make_nil();
    }
    p->err = "Unexpected token while parsing value.";
    return make_nil();
}

// Returns { ok: bool, value: any, error: str }
void sjson_parse_ex(SnaskValue* out, SnaskValue* text_val) {
    if ((int)text_val->tag != SNASK_STR || !text_val->ptr) { *out = make_nil(); return; }
    const char* s = (const char*)text_val->ptr;
    size_t len = strlen(s);
    SjsonP p = {0};
    p.s = s; p.len = len; p.i = 0; p.depth = 0;
    p.max_depth = 128;
    p.max_len = 1024 * 1024; // 1 MiB default
    sjson_ref_init(&p);
    if (len > p.max_len) {
        ...
        r->names[2] = snask_gc_strdup("error"); r->values[2] = make_str(snask_gc_strdup("SJSON input too large."));
        *out = make_obj(r);
        sjson_ref_free(&p);
        return;
    }
    SnaskValue v = sjson_parse_value(&p);
    sjson_skip_ws_and_comments(&p);
    bool ok = (p.err == NULL) && (p.i >= p.len);
    SnaskObject* r = obj_new(3);
    r->names[0] = snask_gc_strdup("ok"); r->values[0] = make_bool(ok);
    r->names[1] = snask_gc_strdup("value"); r->values[1] = ok ? v : make_nil();
    r->names[2] = snask_gc_strdup("error"); r->values[2] = make_str(snask_gc_strdup(ok ? "" : (p.err ? p.err : "Parse error.")));
    *out = make_obj(r);
    sjson_ref_free(&p);
}

// --- SNIF (Snask Interchange Format) ---
// SNIF is the official name for SJSON. The implementation remains the same,
// but we expose SNIF-prefixed native entrypoints so it can be used only via
// `import "snif"` at the language level.
void snif_new_object(SnaskValue* out) { sjson_new_object(out); }
void snif_new_array(SnaskValue* out) { sjson_new_array(out); }
void snif_parse_ex(SnaskValue* out, SnaskValue* text_val) { sjson_parse_ex(out, text_val); }
void snif_type(SnaskValue* out, SnaskValue* v) { sjson_type(out, v); }
void snif_arr_len(SnaskValue* out, SnaskValue* arr) { sjson_arr_len(out, arr); }
void snif_arr_get(SnaskValue* out, SnaskValue* arr, SnaskValue* idx_val) { sjson_arr_get(out, arr, idx_val); }
void snif_arr_set(SnaskValue* out, SnaskValue* arr, SnaskValue* idx_val, SnaskValue* value) { sjson_arr_set(out, arr, idx_val, value); }
void snif_arr_push(SnaskValue* out, SnaskValue* arr, SnaskValue* value) { sjson_arr_push(out, arr, value); }
void snif_path_get(SnaskValue* out, SnaskValue* root, SnaskValue* path) { sjson_path_get(out, root, path); }
...
void sjson_path_get(SnaskValue* out, SnaskValue* root, SnaskValue* path_val) {
    ...
    while (*p) {
        ...
        if ((int)cur.tag != SNASK_OBJ || !cur.ptr) {
            ...
            r->names[0] = snask_gc_strdup("ok"); r->values[0] = make_bool(false);
            r->names[1] = snask_gc_strdup("value"); r->values[1] = make_nil();
            r->names[2] = snask_gc_strdup("error"); r->values[2] = make_str(snask_gc_strdup("path_get: target is not an object/array."));
            *out = make_obj(r);
            return;
        }
        ...
        if (seg[0] == '\0') {
            ...
            r->names[0] = snask_gc_strdup("ok"); r->values[0] = make_bool(false);
            r->names[1] = snask_gc_strdup("value"); r->values[1] = make_nil();
            r->names[2] = snask_gc_strdup("error"); r->values[2] = make_str(snask_gc_strdup("path_get: empty segment."));
            *out = make_obj(r);
            return;
        }
        ...
        if (!found) {
            ...
            r->names[1] = snask_gc_strdup("value"); r->values[1] = make_nil();
            char msg[512];
            snprintf(msg, sizeof(msg), "path_get: segment '%s' not found.", seg);
...
static SnaskValue jp_parse_object(JsonParser* p) {
    ...
    while (p->s[p->i]) {
        ...
        if (!jp_consume(p, ':')) { p->err = "Expected ':' after JSON object key."; free(key); free(obj->names); free(obj->values); free(obj); return make_nil(); }
...
SNASK_ALIAS0(snif_new_object)
SNASK_ALIAS0(snif_new_array)
SNASK_ALIAS1(snif_parse_ex)
SNASK_ALIAS1(snif_type)
SNASK_ALIAS1(snif_arr_len)
SNASK_ALIAS2(snif_arr_get)
SNASK_ALIAS3(snif_arr_set)
SNASK_ALIAS2(snif_arr_push)
SNASK_ALIAS2(snif_path_get)
